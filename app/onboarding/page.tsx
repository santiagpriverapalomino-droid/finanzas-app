...
export default function Onboarding() {
  ...
  const features = [
    { icon: '🤖', title: 'Asesor IA personalizado', desc: 'Pregúntale cualquier cosa sobre finanzas y te responde en segundos.' },
    ...
    { icon: '📄', title: 'Exporta en PDF', desc: 'Genera un reporte mensual de tus finanzas.' },
  ]
  ...
  return (
    <div className="min-h-screen bg-[#f5f3ee]">
      ...
      {/* PASO 0: Bienvenida */}
      {step === 0 && (
        <div className="flex flex-col px-5 pt-10 pb-6 min-h-screen">
          <div className="flex flex-col items-center text-center mb-6">
            <div className="w-16 h-16 rounded-[20px] bg-[#5a4bc3] flex items-center justify-center mb-3 shadow-lg">
            ...
            <h1 className="text-[24px] font-bold text-[#1f1f1f] mb-1">Hola, {firstName} 👋</h1>
            <p className="text-[#5d594f] text-[14px] leading-relaxed">
              Bienvenido a <span className="font-bold text-[#5a4bc3]">Finti</span> — tu gestor financiero inteligente para jóvenes peruanos.
            </p>
          </div>

          <div className="space-y-2 mb-6">
            {features.map(f => (
              <div key={f.title} className="flex items-center gap-3 bg-white rounded-[16px] px-4 py-3 border border-[#ebe6db]">
                <span className="text-xl flex-shrink-0">{f.icon}</span>
                <div>
                  <p className="text-[13px] font-bold text-[#1f1f1f]">{f.title}</p>
                  <p className="text-[11px] text-[#8c887d]">{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <button onClick={() => setStep(1)}
            className="w-full bg-[#5a4bc3] text-white rounded-[16px] py-4 font-bold text-[15px] shadow-lg">
            Empezar — son solo 2 minutos 🚀
          </button>
          <p className="text-center text-[11px] text-[#8c887d] mt-2">100% gratis · Sin tarjeta de crédito</p>
        </div>
      )}
        ...
        <div className="flex flex-col px-6 pt-12 min-h-screen">
          ...
          <div className="space-y-4 mb-8">
            <div>
              ...
              <input type="number" placeholder="Ej: 2500" value={ingreso}
          ...
          <button onClick={() => ingreso && diaCobro && setStep(2)}
        ...
        <div className="flex flex-col px-6 pt-12 min-h-screen">
          ...
          <div className="space-y-3 mb-4">
            <button onClick={() => setEsPrimerSueldo(true)}
              ...
              <p className="text-[13px] text-[#5d594f] mt-1">50% necesidades, 30% gustos, 20% ahorro.</p>
            </button>
          ...
          {esPrimerSueldo === true && (
            <div className="rounded-[16px] bg-[#ede9ff] border border-[#c8bbf5] p-4 mb-4">
