...
async function getCotizacion(s: any) {
  try {
    ...
    if (s.fx) {
      const res = await fetch(`https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=USD&to_currency=PEN&apikey=${process.env.ALPHA_VANTAGE_KEY}`, { cache: 'no-store' })
      const data = await res.json()
      const rate = data['Realtime Currency Exchange Rate']
      precio = parseFloat(rate?.['5. Exchange Rate'] || '0')
    } else if (s.cripto) {
      const res = await fetch(`https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=BTC&to_currency=USD&apikey=${process.env.ALPHA_VANTAGE_KEY}`, { cache: 'no-store' })
      const data = await res.json()
      const rate = data['Realtime Currency Exchange Rate']
      precio = parseFloat(rate?.['5. Exchange Rate'] || '0')
    } else {
      const res = await fetch(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${s.symbol}&apikey=${process.env.ALPHA_VANTAGE_KEY}`, { cache: 'no-store' })
      const data = await res.json()
